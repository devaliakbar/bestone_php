...
$(document).ready(function () { 
   loadBookings();
   var checkBooking = setInterval(checkBookingFunction, 25000);
});
...
function loadBookings(){
    ...
    loadStockHTTP.onreadystatechange = function () {
                ...
                $('table tr').click(function () {
                    ...
                    if (itemIndex > 0) {
                        var itemId = $('table tr:eq(' + itemIndex + ')').attr('class').split(' ')[0];
                         ...
                         var url = "user-manage-orders.php?q=" + itemId;
        ...
        loadStockHTTP.open("POST", prefixLink + "currentOrder.php", true);
    
    loadStockHTTP.send();
}

function checkBookingFunction() {
	loadBookings();
}